...
describe("DomainsManager: backward compatibility and domain enabling", () => {
  ...
  describe("constructor", () => {
    it("enables all domains when no argument is provided (default behavior)", () => {
      ...
      expect(enabledDomains.size).toBe(11);
    ...
    it("enables all domains when undefined is passed as argument", () => {
      ...
      expect(enabledDomains.size).toBe(11);
      expect(Array.from(enabledDomains).sort()).toEqual(["advanced-security", "core", "pipelines", "releases", "repositories", "search", "test-plans", "tfvc", "wiki", "work", "work-items"]);
    });
  ...
  describe("string input handling", () => {
    ...
    it("Error and enables all domains when an invalid domain name string is passed", () => {
      ...
      expect(errorSpy).toHaveBeenCalledWith(
        "Error: Specified invalid domain 'invalid-domain'. Please specify exactly as available domains: advanced-security, pipelines, core, releases, repositories, search, test-plans, tfvc, wiki, work, work-items"
      );
    });
  });
  ...
  describe("getAvailableDomains method", () => {
    it("returns the full list of available domains", () => {
      ...
      expect(availableDomains).toEqual(["advanced-security", "pipelines", "core", "releases", "repositories", "search", "test-plans", "tfvc", "wiki", "work", "work-items", "mcp-apps"]);
      expect(availableDomains.length).toBe(12);
    });
  });
  ...
  describe("edge cases and additional coverage", () => {
    ...
    it("tests comma-separated string input with 'all' keyword", () => {
      ...
      const enabledDomains = manager.getEnabledDomains();
      expect(enabledDomains.size).toBe(11); // Should enable all because 'all' is present
    });
  });
